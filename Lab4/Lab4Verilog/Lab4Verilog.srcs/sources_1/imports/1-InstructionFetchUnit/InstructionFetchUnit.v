...
module InstructionFetchUnit(Instruction, PCResult, Reset, Clk);
   ...
   output wire [31:0] PCResult; //this is the 32 bit output. 
   wire [31:0] PCAddResult; // wire for the add result of Program Counter
   
   ProgramCounter pc(PCAddResult, PCResult, Reset, Clk); // call to program counter
   
   PCAdder pa(PCResult, PCAddResult); //call to the adder with the PCResult and PCAddResult
   
   InstructionMemory im(PCResult, Instruction); //call to instruction memory
